...
//
// Server voice command backend — stub implementation.
//
// The server engine has no audio input, so speech recognition is not
// supported. startListening sets the result to a "not supported" string
// and returns false. All other entry points are no-ops.
//
...
bool MCPlatformStartListening(MCStringRef /*p_language*/)
{
    ...
    /* UNCHECKED */ MCStringCreateWithCString(
        "startListening: speech recognition is not supported in server mode", t_err);
