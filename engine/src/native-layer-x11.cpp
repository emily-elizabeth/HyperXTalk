...
// after a fresh webkit_web_view_get_snapshot() surface has been stored.
// Schedules a Redraw() so doPaint() runs and blits the snapshot.
...
void MCNativeLayerX11::doAttach()
{
    if (m_child_window == NULL)
    {
        ...
        if (m_browser_widget != NULL)
        {
            ...
            // Register the snapshot-ready repaint bridge so libbrowser's
            // on_snapshot_done() can schedule a Redraw() after storing a snapshot.
