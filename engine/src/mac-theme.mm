...
bool MCPlatformGetControlThemePropColor(MCPlatformControlType p_type, MCPlatformControlPart p_part, MCPlatformControlState p_state, MCPlatformThemeProperty p_which, MCColor& r_color)
{
        ...
        case kMCPlatformThemePropertyTextColor:
        {
            ...
            else
            {
                switch (p_type)
                {
                    ...
                    case kMCPlatformControlTypeTabButton:
                    {
                        // Selected tab text is always white (selectedMenuItemTextColor)
                        // so it contrasts against the accent-colour fill drawn by
                        // MCThemeDraw / mac-stubs-arm64.mm.  The macOS-10.16 guard
                        // that previously forced controlTextColor (dark) on Big Sur+
                        // is removed: ARM64 Macs always run macOS 11+, and the
                        // accent-colour background needs white text regardless of
                        // OS version.
                        if (p_state & kMCPlatformControlStateSelected)
    ...
    if (t_found && t_color != nil)
    {
        if (t_is_pattern)
        {
            // Patterns not supported at the moment — use controlHighlightColor as
            // a solid approximation.  Apply the same appearance-aware resolution
            // used for non-pattern colours so the conversion doesn't yield nil on
            // macOS 11+ dynamic appearances.
            NSColor *t_highlight = [NSColor controlHighlightColor];
            NSColor *t_solid = [t_highlight colorUsingColorSpace:[NSColorSpace sRGBColorSpace]];
            if (t_solid == nil)
            {
                __block NSColor *t_blk = t_highlight;
                [[NSApp effectiveAppearance] performAsCurrentDrawingAppearance:^{
                    t_blk = [t_blk colorUsingColorSpace:[NSColorSpace sRGBColorSpace]];
                }];
                t_solid = t_blk;
            }
            if (t_solid == nil)
                t_solid = [t_highlight colorUsingColorSpaceName:NSCalibratedRGBColorSpace];
            t_color = t_solid;
        }
        else
        {
            // On macOS 11+, semantic/dynamic system colours (controlShadowColor,
            // selectedControlColor, etc.) cannot be directly converted with
            // colorUsingColorSpaceName: — it returns nil unless the call is made
            // inside performAsCurrentDrawingAppearance:.  Try the modern sRGB
            // path first; if that also returns nil (can happen outside an
            // appearance context), resolve against the app's effective appearance.
            NSColor *t_resolved = [t_color colorUsingColorSpace:[NSColorSpace sRGBColorSpace]];
            if (t_resolved == nil)
            {
                __block NSColor *t_block = t_color;
                [[NSApp effectiveAppearance] performAsCurrentDrawingAppearance:^{
                    t_block = [t_block colorUsingColorSpace:[NSColorSpace sRGBColorSpace]];
                }];
                t_resolved = t_block;
            }
            // Final fallback: calibrated RGB (deprecated but still works for
            // colours that are already concrete).
            if (t_resolved == nil)
                t_resolved = [t_color colorUsingColorSpaceName: NSCalibratedRGBColorSpace];
            t_color = t_resolved;
        }
        ...
        {
            // Colour could not be resolved — signal failure so callers fall
            // back to their own default colour handling.
            t_found = false;
        }
    }
