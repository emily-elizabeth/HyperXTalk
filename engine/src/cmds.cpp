                                            ...
                                            MCChunk*                p_dst)
{
    ...
    // Locate the starting dest paragraph.
    findex_t t_dst_si = (t_dst_type == CT_UNDEFINED) ? 0 : (findex_t)t_dst_ocp.mark.start;
    findex_t t_dst_ei = (t_dst_type == CT_UNDEFINED) ? INT32_MAX : (findex_t)t_dst_ocp.mark.finish;
    ...
    MCParagraph* t_src = t_src_start;
    MCParagraph* t_dst = t_dst_start;
    for (;;)
    {
        ...
        MCParagraph* t_next_src = (MCParagraph*)t_src->next();
        MCParagraph* t_next_dst = (MCParagraph*)t_dst->next();
        if (t_next_src == t_src_list || t_next_dst == t_dst_list)
            break;
        t_src = t_next_src;
        t_dst = t_next_dst;
    }
